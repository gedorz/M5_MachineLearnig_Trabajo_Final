...
def init_db() -> None:
    ...
    """
    with _get_postgres_connection_server(host="postgresm5", db_name="dbPredicterManager") as connection:
        with connection.cursor() as cursor:
            cursor.execute(query)
        connection.commit()
    logger.info("event=init_db table=tasks,datasets status=ready")
    
# Carga la base de datos 
def get_db() -> Generator[Any, None, None]:
    connection = None   # _get_postgres_connection_server(host="postgresm5", db_name="dbPredicterManager")
    try:
        yield connection
    finally:
        connection.close()    
...
def get_db_tasks() -> Generator[Any, None, None]:
    connection = _get_postgres_connection_server(host="postgresm5", db_name="dbPredicterManager")
    try:
        yield connection
    finally:
        connection.close()           